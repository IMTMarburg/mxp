...
def read_mxp(filename,
    ...
    amplification_data = extract_amplification_curves(ole, fileformat,
                                                      allowed_cycles,
                                                      len(assay_names))
...
def extract_well_names_and_assay_names(ole, fileformat, empty_assays_ok=False):
    ...
    if len(well_parts) != 96:
        raise ValueError("Did not find 96 well parts in Storage0 - was %i" %
                         len(well_parts))
    well_names = []
...
def extract_amplification_curves(ole,
                                 fileformat,
                                 allowed_cycles,
                                 supposed_wells=96):
    ...
    y[-1] = y[
        -1][:
            block_len]  # there is a different seperator after the last one, so we set it to the first length

    if max(set([len(a) for a in y])) > 966:  #816: #572: #476: #466: #428:
        #for ii, a in enumerate(y):
        #print ii, len(a)
        raise ValueError(
            "Seen a very large chunk: %i " % max(set([len(a) for a in y])))
    result_fluorescence = []
    result_temperatures = []
        ...
        for block_id, block in enumerate(y):
            if block is not False:
                ...
                if len(numbers) != no_of_cycles:
                    raise ValueError(
                        "Not exactly 40 datapoints in amplification curve")
                result_fluorescence.append(numbers)
                temperatures = np.array([
                    to_16_bit(block[3 + 4 + offset * 10:3 + 4 + 2 +
                                    offset * 10]) / 10.0
                    for offset in xrange(0, no_of_cycles)
                ])
    ...
    elif block_len in (724, 726, 748,
                       796):  # blocks with two samples per cycle
        for block_id, block in enumerate(y):
            if block is not False:
                if block_id > supposed_wells:
                    break
                ...
                result_fluorescence.append(numbers)
                temperatures = np.array([
                    all_numbers[x * 9 + 2] / 10.0
                    for x in xrange(0, no_of_cycles)
                ])
                if (temperatures > 120).any():
                    raise ValueError(
                        "Temperature above 120 degrees obsereved. Most likely a parsing error"
    ...
    if len(result_fluorescence) != supposed_wells:
        raise ValueError("Not exactly the supposed %i wells - was %i" %
