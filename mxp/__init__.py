...
This package supports basic files, where a single channel has been read.
...
def read_mxp(filename, allowed_cycles = (4, 7, 13, 14,25, 28, 30, 34, 35, 36, 40, 41, 43, 44, 45, 49, 50, 55, 60, 65, 120), empty_assays_ok = False):
    ...
    amplification_data = extract_amplification_curves(ole, fileformat, allowed_cycles, len(assay_names))
    ...
    res['Well Key'] = pd.Categorical(res['Well Key'], natsort.humansorted(set(
        [well_no_to_code(x) for x in xrange(1, 97)])))
    if (res['Temperature'] > 120).any():
        raise ValueError("Temperature above 120 degrees obsereved. Most likely a parsing error")
    return res
...
def extract_well_names_and_assay_names(ole, fileformat, empty_assays_ok = False):
    ...
    if len(well_parts) != 96:
        raise ValueError("Did not find 96 well parts in Storage0 - was %i" % len(well_parts))
    well_names = []
...
def extract_amplification_curves(ole, fileformat, allowed_cycles, supposed_wells = 96):
    ...
    if fileformat == 0: # I really which I had a smarter way to do this
        ...
        if no_of_cycles == -1:
            offset = offset - 1
            no_of_cycles = ord(x[offset][9])
    else:
        raise ValueError("Unknown fileformat")
    if no_of_cycles not in allowed_cycles:
        print "%x" % x[4].find("("), fileformat
        raise ValueError("File contained an unexpected number of cycles: %i. Allowed: %s" % (no_of_cycles, allowed_cycles))
    seperator = "\x00\x00\x00" + chr(no_of_cycles)
    y = x[offset]
    while (y.count(seperator) < supposed_wells):
        offset += 1
        y = y + "\x00\x00\x00\x60" +  x[offset]
    y = y.split(seperator)
    ...
    for block_id, block in enumerate(y):
        if block is False:
            continue
        start_id = block_id
        while len(y[start_id]) < block_len and (block_id + 1 < len(y) -1): # sometimes the splitter appers in the data. then we patch it up.
    ...
    if block_len < 724:
        for block_id, block in enumerate(y):
            if block is not False:
                if block_id > supposed_wells:
                    break
                numbers = [to_16_bit(block[3+offset*10:3+2+offset*10]) for offset in xrange(0, no_of_cycles)]
                ...
                temperatures = np.array([to_16_bit(block[3+4+offset*10:3+4+2+offset*10]) / 10.0 for offset in xrange(0, no_of_cycles)])
        ...
        for block_id, block in enumerate(y):
            if block is not False:
                ...
                all_numbers = [to_16_bit(block[3+offset*2:3+2+offset*2]) for offset in xrange(0, no_of_cycles * 9) ]
                numbers = [all_numbers[x * 9] for x in xrange(0, no_of_cycles)]
                if len(numbers) != no_of_cycles:
                    raise ValueError("Not exactly 40 datapoints in amplification curve")
                result_fluorescence .append(numbers)
                temperatures = np.array([all_numbers[x * 9 + 2] / 10.0 for x in xrange(0, no_of_cycles)] )
                if (temperatures > 120).any():
                    raise ValueError("Temperature above 120 degrees obsereved. Most likely a parsing error")
                result_temperatures.append(temperatures)
    elif block_len == 806:
        for block in y:
            ...
            result_fluorescence .append(numbers)
            temperatures = np.array([all_numbers[x * 5 + 2] / 10.0 for x in xrange(0, no_of_cycles)] )
            result_temperatures.append(temperatures)
            if (temperatures > 120).any():
                    raise ValueError("Temperature above 120 degrees obsereved. Most likely a parsing error")
        ...
        for block in y:
            ...
            result_fluorescence .append(numbers)
            temperatures = np.array([all_numbers[x * 6 + 2] / 10.0 for x in xrange(0, no_of_cycles)] )
            result_temperatures.append(temperatures)
            if (temperatures > 120).any():
                raise ValueError("Temperature above 120 degrees obsereved. Most likely a parsing error")
    else:
    ...
    if len(result_fluorescence) != supposed_wells:
        raise ValueError("Not exactly the supposed %i wells - was %i" % (supposed_wells, len(result_fluorescence)))
